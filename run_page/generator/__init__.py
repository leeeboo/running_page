...
class Generator:
    ...
    def sync_from_data_dir(
        ...
        existing_starts = (
            {
                ...
                for (start,) in self.session.query(Activity.start_date)
                if start
            }
        ...
        for t in tracks:
            activity = t.to_namedtuple(run_from=file_suffix)
            start = arrow.get(activity.start_date).int_timestamp
            # COROS FIT IDs differ from Strava IDs for the same recorded activity.
            if deduplicate_by_start_time and start in existing_starts:
